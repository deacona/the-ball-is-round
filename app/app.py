...
data_pivot = pd.pivot_table(data, values="score", index="file", columns="category", aggfunc=np.mean, margins=False)
# print(data_pivot)
...
fig.update_layout(
    ...
    yaxis=dict(
        title="<b>File</b>", autorange='reversed'
    ),
    )
...
app.layout = html.Div(
    children=[
        html.Div(
            children=[
                html.H1(
                    children="Data Quality Dashboard", className="header-title"
                ),
                html.P(
